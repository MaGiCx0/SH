...
module.exports = {
	...
	async execute(client, interaction) {
       let primeR = "1124354439734640650" // رتبة البرايم
        ...
        if (interaction.options.getSubcommand() === "subscribe" && gID) {
            ...
            if(b.customId === "1mP"){
                ...
                await coinsdb.subtract(`coins_${interaction.user.id}` , 750)
                 ...
                 let PU = await client.guilds.cache.get("1104015122684452935").members.cache.find(m => m.id == interaction.user.id)
                if(!PU) {
                    await interaction.editReply({ content : `**🥳 You have been purchased ShareBot Prime for \`${humanizeDuration(time.month , { units : ["mo"], round : true})}\`**` , components : []})
				let mmsg = new WebhookClient({ id: "1034204289893138545", token: "BGLLbaOI1xWIZOSmsTslP8A2onP-yQ9mo7YhNZH58CkwTs6o-3HeHv5k-DVBJUU7D7xS" });
            ...
            if(b.customId === "3mP"){
                ...
                await coinsdb.subtract(`coins_${interaction.user.id}` , 2250)
                 ...
                 let PU = await client.guilds.cache.get("1104015122684452935").members.cache.find(m => m.id == interaction.user.id)
                if(!PU) {
                    await interaction.editReply({ content : `**🥳 You have been purchased ShareBot Prime for \`${humanizeDuration(time.three_months , { units : ["mo"], round : true})}\`**` , components : []})
				let mmsg = new WebhookClient({ id: "1124439236796485683", token: "0TXJPzyJRex8b5YqH0l2pJSsvMbA_JvP3hYta7zlyJ3bK-ylHwa1k8sHqGvJ8SX2Y-rE" });
                return mmsg.send(`**A new prime subscription.**\nBy: <@${interaction.user.id}>, ${interaction.user.id}, [${interaction.user.tag}](https://discord.com/user/${interaction.user.id})\nServer: ${guild.name} | ${guild.id}\nTime: 3 Month`)
                }
                if(PU){
                    await interaction.editReply({ content : `**🥳 You have been purchased ShareBot Prime for \`${humanizeDuration(time.three_months , { units : ["mo"], round : true})}\`**` , components : []})
...
if (interaction.options.getSubcommand() === "preview") {
        ...
        if(primeS !== null){
          ...
          if(G !== null){
            ...
            if(end > Date.now()){
            await interaction.editReply(`**🗂️ Server information: \`\n${G} \n- ${GN || "Unknown"}\` \n⏳ Ends At: \`${humanizeDuration(endT , { round : true })}\` **`)
        }else
        if(end < Date.now()){  
